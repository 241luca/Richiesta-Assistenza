...
export class ApiKeyService {
  ...
  async upsertApiKey(
    data: ApiKeyInput,
    organizationId: string, // Mantenuto per compatibilità API ma non usato
    userId: string
  ): Promise<ApiKey> {
    try {
   ...
   * Elimina una API key
   */
  async deleteApiKey(service: ApiKeyInput['service'], organizationId?: string): Promise<boolean> {
    try {
   ...
   * Testa una API key
   */
  async testApiKey(service: ApiKeyInput['service'], organizationId?: string): Promise<{
    success: boolean;
    message: string;
    details?: any;
  }> {
