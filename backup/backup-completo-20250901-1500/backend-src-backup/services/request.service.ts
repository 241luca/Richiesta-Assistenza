...
export class RequestService {
  ...
  async create(data: {
    ...
    try {
      ...
      if (request.client && !request.client.fullName) {
        request.client.fullName = `${request.client.firstName || ''} ${request.client.lastName || ''}`.trim();
      }
    ...
    try {
      ...
      const request = await prisma.assistanceRequest.update({
        ...
        data: {
          ...
          requestedDate: data.requestedDate ? new Date(data.requestedDate) : undefined,
          assignedDate: data.professionalId && !existing.professionalId ? new Date() : undefined,
        },
        include: {
          client: {
            select: {
      ...
      if (request.professional && !request.professional.fullName) {
        request.professional.fullName = `${request.professional.firstName || ''} ${request.professional.lastName || ''}`.trim();
      }
    ...
    try {
      ...
      return quotes.map(quote => ({
        ...
        professional: quote.professional ? {
          ...quote.professional,
          fullName: quote.professional.fullName || `${quote.professional.firstName || ''} ${quote.professional.lastName || ''}`.trim()
        } : null,
      }));
    } catch (error) {
