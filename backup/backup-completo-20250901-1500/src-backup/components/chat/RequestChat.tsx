...
const RequestChat: React.FC<RequestChatProps> = ({ 
  ...
  const { data: chatAccess } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get(`/chat/${requestId}/access`);
      return response.data.data;
    }
  });
  ...
  const editMessageMutation = useMutation({
    mutationFn: async ({ messageId, message }: { messageId: string; message: string }) => {
      const response = await apiClient.put(`/chat/messages/${messageId}`, { message });
      return response.data.data;
    },
    onSuccess: () => {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: async () => {
      const response = await apiClient.post(`/chat/${requestId}/mark-read`);
