    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (isAdminView && professionalId) {
          ...
          const response = await apiClient.get('/user/subcategories');
          return response.data;
        }
      } catch (error) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (subcategoryIds: string[]) => {
      if (isAdminView && professionalId) {
        ...
        const response = await apiClient.put(`/user/subcategories/${professionalId}`, {
          subcategoryIds
        });
        return response.data;
      } else {
        // Professionista che aggiorna le proprie sottocategorie
        const response = await apiClient.put('/user/subcategories', {
          subcategoryIds
        });
        return response.data;
      }
