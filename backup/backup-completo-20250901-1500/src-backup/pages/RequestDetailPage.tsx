...
export default function RequestDetailPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async (newStatus: string) => {
      await apiClient.patch(`/requests/${id}/status`, { status: newStatus });
    },
    onSuccess: () => {
  ...
  const handleDownloadPDF = async () => {
    try {
      const response = await apiClient.get(`/requests/${id}/pdf`, {
        responseType: 'blob'
      });
      ...
      apiClient.get(`/maps/geocode`, {
        ...
        .then(response => {
          ...
          if (response.data && response.data.latitude && response.data.longitude) {
            ...
            return apiClient.patch(`/requests/${id}/coordinates`, {
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="flex items-center space-x-4 mt-2">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${priorityData.color}`}>
                {priorityData.label}
              </span>
              ...
              <div className="space-y-2">
                {request.attachments.map((attachment: any) => (
                  <div key={attachment.id} className="flex items-center p-3 border rounded-lg hover:bg-gray-50">
                    ...
                    <span className="text-gray-900 flex-1">{attachment.originalName || attachment.fileName}</span>
              ...
              <div className="space-y-3">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    {request.professional.profession && (
                      <p className="text-sm text-gray-600">{request.professional.profession}</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center">
