...
export function SubcategoriesPage() {
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ id, isActive }: { id: string; isActive: boolean }) => {
      await apiClient.put(`/subcategories/${id}`, { isActive });
    },
    onSuccess: () => {
...
function SubcategoryFormModal({ 
  ...
  const createMutation = useMutation({
    mutationFn: async (data: any) => {
      if (subcategory) {
        return await apiClient.put(`/subcategories/${subcategory.id}`, data);
      } else {
        return await apiClient.post('/subcategories', data);
      }
    },
    onSuccess: () => {
...
function AiSettingsModal({ 
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: AiSettings) => {
      return await apiClient.post(`/subcategories/${subcategory.id}/ai-settings`, data);
    },
    onSuccess: () => {
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        const response = await apiClient.post(`/kb-documents/upload`, formData, {
                    ...
                    {kbDocuments.map((doc) => (
                      <div key={doc.id} className="flex items-center justify-between p-2 bg-white rounded border">
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              await apiClient.delete(`/kb-documents/${doc.id}`);
