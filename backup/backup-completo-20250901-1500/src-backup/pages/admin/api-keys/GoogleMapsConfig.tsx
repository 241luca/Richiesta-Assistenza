...
export default function GoogleMapsConfig() {
  ...
  const { data: apiKey, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get('/admin/api-keys/GOOGLE_MAPS');
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      return await apiClient.post('/admin/api-keys', {
        service: 'GOOGLE_MAPS',
        ...data
      });
    },
  ...
  const testMutation = useMutation({
    mutationFn: async () => {
      return await apiClient.post('/admin/api-keys/GOOGLE_MAPS/test');
    },
    onSuccess: (data) => {
