...
export class WhatsAppRealtimeService {
   ...
   */
  private async findClientByPhone(phoneNumber: string): Promise<ClientUser | null> {
    try {
      ...
      return user as ClientUser | null;
   ...
   */
  private async handleClientMessage(client: ClientUser, message: SavedMessage): Promise<void> {
    try {
      ...
      if (openRequest) {
        ...
        if (this.looksLikeAssistanceRequest(message.message)) {
          ...
          // Notifica admin
          await this.notifyNewRequest(newRequest as AssistanceRequest, client);
   ...
   */
  private async notifyNewRequest(request: AssistanceRequest, client: ClientUser): Promise<void> {
    try {
      const admins = await prisma.user.findMany({
        where: {
          role: { in: ['ADMIN', 'SUPER_ADMIN'] },
   ...
   */
  private async sendUnreadMessages(userId: string, socket: Socket): Promise<void> {
    try {
