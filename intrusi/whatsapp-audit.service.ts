...
export interface WhatsAppAuditEvent {
  ...
  kbDocuments?: number;
  errorMessage?: string;
  metadata?: Record<string, unknown>; // ✅ Invece di any
}
...
interface AuditReportActionBreakdown {
  action: string;
  count: number;
}
...
interface AuditReport {
  ...
  actionBreakdown: AuditReportActionBreakdown[];
...
type AuditCategory = 'AUTH' | 'DATA' | 'ADMIN' | 'SYSTEM' | 'SECURITY' | 'BUSINESS' | 'LEGAL' | 'COMMUNICATION';
