...
echo "🔧 CORREZIONE COMPLETA MAPPATURA CAMPI"
echo "======================================"
echo ""

# Crea versione corretta del service con mappatura corretta
cat > backend/src/services/simple-backup.service.fixed.ts << 'EOF'
...
class SimpleBackupService {
  ...
  async backupDatabase(userId: string): Promise<BackupResult> {
    ...
    try {
      ...
      // Ritorna con mappatura corretta
      return {
        id: backup.id,
  ...
  async listBackups(type?: BackupType): Promise<BackupResult[]> {
    try {
      ...
      // Converti per il frontend con mappatura corretta
  ...
  async getBackupStats(): Promise<any> {
    try {
      ...
      const backups = await prisma.backup.findMany({
        select: { filepath: true, file_size: true }
      });
...
mv backend/src/services/simple-backup.service.fixed.ts backend/src/services/simple-backup.service.ts

echo "✅ Service corretto con mappatura snake_case"
echo ""
