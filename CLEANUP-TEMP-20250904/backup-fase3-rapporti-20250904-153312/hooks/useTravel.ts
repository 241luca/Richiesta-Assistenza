...
export const useTravel = () => {
  ...
  const updateWorkAddressMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      // Aggiorna cache
      queryClient.setQueryData(TRAVEL_QUERY_KEYS.workAddress, data);
      
      // Invalida tutte le query viaggi correlate
      queryClient.invalidateQueries({ 
        queryKey: ['travel', 'request'],
        exact: false 
      });
      queryClient.invalidateQueries({ 
        queryKey: ['travel', 'batch'],
        exact: false 
      });
      
      // Toast gestito dal componente, non qui
    },
    onError: (error: any) => {
      console.error('Update work address error:', error);
      // Error toast gestito dal componente
    }
  });
