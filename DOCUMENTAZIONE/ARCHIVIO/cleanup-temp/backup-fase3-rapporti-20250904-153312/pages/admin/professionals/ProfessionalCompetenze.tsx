...
export default function ProfessionalCompetenze() {
  ...
  const removeCompetenceMutation = useMutation({
    mutationFn: async (competenceToRemove: any) => {
      // Dobbiamo rimuovere la sottocategoria dall'array e fare PUT con l'array aggiornato
      const subcategoryIdToRemove = competenceToRemove.subcategoryId || competenceToRemove.Subcategory?.id || competenceToRemove.subcategory?.id;
  ...
  // Filtra sottocategorie già assegnate
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid gap-4">
        {assignedCompetences.map((item: any) => {
          // Estrai i dati dalla struttura corretta
          const subcategoryName = item.Subcategory?.name || 'Nome non disponibile';
          const categoryName = item.Subcategory?.category?.name || 'Categoria non disponibile';
          const description = item.Subcategory?.description || '';
