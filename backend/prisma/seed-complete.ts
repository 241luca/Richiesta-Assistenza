...
async function seed() {
  try {
    ...
    // Array per salvare gli utenti creati
    const users = {
      admins: [],
      clients: [],
      professionals: []
    ...
    for (const reqData of requestsData) {
      const request = await prisma.assistanceRequest.create({
        data: {
          id: randomUUID(),
          ...reqData,
          address: users.clients.find(c => c.id === reqData.clientId)?.address,
          city: users.clients.find(c => c.id === reqData.clientId)?.city,
          province: users.clients.find(c => c.id === reqData.clientId)?.province,
          postalCode: users.clients.find(c => c.id === reqData.clientId)?.postalCode,
          createdAt: now,
          updatedAt: now
        }
      })
