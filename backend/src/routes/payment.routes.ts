...
// 🔒 Protegge tutte le routes dei pagamenti
// Se il modulo 'payments' è disabilitato, blocca l'accesso con 403
...
// GET /api/payments/my-payments - I miei pagamenti (per clienti)
router.get('/my-payments', authenticate, async (req: any, res) => {
  try {
    ...
    // Costruisci filtri - solo i pagamenti del cliente loggato
    const where: any = {
      clientId: user.id
    };
...
router.get('/', authenticate, async (req: any, res) => {
  try {
    ...
    if (status) where.status = status;
    if (type) where.type = type;
    if (method) where.paymentMethod = method;  // CORRETTO: paymentMethod!
...
router.post('/create-intent', authenticate, async (req: any, res) => {
  try {
    ...
    const payment = await paymentService.createPayment({
      amount,
      type: metadata?.type || 'FINAL_PAYMENT',
      method: 'CARD',  // Questo resta 'method' perché lo schema lo vuole così
...
router.post('/test-connection', authenticate, requireRole([Role.ADMIN, Role.SUPER_ADMIN]), async (req: any, res) => {
  try {
    ...
    if (result) {
      ...
      res.json(ResponseFormatter.error('Connessione Stripe fallita', 'STRIPE_CONNECTION_FAILED'));
    }
  } catch (error: any) {
    logger.error('Test connection error:', error instanceof Error ? error.message : String(error));
    res.json(ResponseFormatter.error(error instanceof Error ? error.message : String(error) || 'Test connessione fallito', 'STRIPE_TEST_FAILED'));
  }
});
