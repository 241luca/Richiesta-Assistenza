...
router.get('/status', async (req: Request & { app: any }, res: Response): Promise<Response> => {
  try {
    ...
    try {
      const hasCalendarCredentials = await GoogleApiKeyService.hasValidCredentials();
      services.push({
    ...
    const systemStats = {
      cpu: {
        ...
        usage: Math.min(100, Math.round((cpuUsage.user + cpuUsage.system) / 10000)), // Percentuale approssimativa
        loadAvg: os.loadavg()
      },
      memory: {
        total: totalMem,
      ...
      disk: {
        // Node.js non ha API native per disk space, restituiamo placeholder
        total: 'N/A',
        used: 'N/A',
        free: 'N/A'
      },
