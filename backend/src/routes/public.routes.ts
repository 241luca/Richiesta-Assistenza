 ...
 * 🆕 Ottieni Google Maps API Key dal database
 * Endpoint pubblico per caricare la chiave dinamicamente nel frontend
 */
router.get('/config/google-maps-key', async (req, res) => {
  try {
    ...
    if (!setting || !setting.value) {
      logger.warn('Google Maps API Key not found in database');
    ...
    return res.json(
      ResponseFormatter.success(
        ...
        'Google Maps API Key retrieved successfully'
      )
    );
  } catch (error: any) {
