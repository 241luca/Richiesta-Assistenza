...
class HealthCheckService {
  ...
  private async checkNotificationSystem(): Promise<HealthCheckResult> {
    ...
    try {
      // 1. Controlla configurazione email (Brevo) - SEMPRE AGGIUNTO
  ...
  private async checkBackupSystem(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      try {
        const backupPath = path.join(require('os').homedir(), 'RichiestaBackup', 'database');
  ...
  private async checkPaymentSystem(): Promise<HealthCheckResult> {
    ...
    try {
      // 1. Controlla configurazione Stripe - SEMPRE AGGIUNTO
  ...
  private async checkAISystem(): Promise<HealthCheckResult> {
    ...
    try {
      // 1. Controlla configurazione OpenAI - SEMPRE AGGIUNTO
