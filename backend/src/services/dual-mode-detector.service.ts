...
interface WhatsAppConfigWithContacts {
  id: string;
  instanceId: string;
...
export class DualModeDetector {
  ...
  async detectSenderType(
    ...
    try {
      ...
      if (!whatsappConfig) {
        logger.error(
          `WhatsApp config not found for instance: ${whatsappInstanceId}`
        );
      ...
      // Controlla se è il numero del professionista stesso
      if (config.professionalPhones.includes(phoneNumber)) {
      ...
      // Controlla se è un numero fidato
      if (config.trustedNumbers.includes(phoneNumber)) {
      ...
      // Controlla se esiste già un contatto classificato
      if (config.contacts && config.contacts.length > 0) {
        const existingContact = config.contacts[0];

        if (existingContact.isVerified) {
          const mode = this.mapContactTypeToMode(
            existingContact.contactType as ContactType
          );
          ...
          return {
            ...
            factors: {
              isRegisteredProfessional:
                existingContact.contactType === ContactType.PROFESSIONAL,
              isTrustedNumber:
                existingContact.contactType === ContactType.TRUSTED,
              isBlacklistedNumber: false,
              languagePatternMatch: false,
      ...
      // Controlla override precedenti per questo numero
      const previousOverride = await this.getLatestOverride(
        whatsappInstanceId,
        phoneNumber
      );
      if (previousOverride) {
        logger.info(
          `Previous override found for ${phoneNumber}: ${previousOverride.overriddenTo}`
        );
        return {
          ...
          factors: {
            isRegisteredProfessional:
              previousOverride.overriddenTo === DetectionMode.PROFESSIONAL,
            isTrustedNumber: false,
            isBlacklistedNumber: false,
            languagePatternMatch: false,
      ...
      // DEFAULT: Considera come CLIENT (più sicuro)
  ...
  async addProfessionalNumber(
    ...
    try {
      ...
      if (!whatsappConfig.professionalPhones.includes(phoneNumber)) {
        ...
        logger.info(
          `Added professional number: ${phoneNumber} to instance: ${instanceId}`
        );
      }

      await this.updateContactClassification(
        instanceId,
        phoneNumber,
  ...
  async addTrustedNumber(
    ...
    try {
      ...
      if (!whatsappConfig.trustedNumbers.includes(phoneNumber)) {
        ...
        logger.info(
          `Added trusted number: ${phoneNumber} to instance: ${instanceId}`
        );
      }
  ...
  async removeFromLists(
    ...
    try {
      ...
      const updatedProfessionalPhones =
        whatsappConfig.professionalPhones.filter((p) => p !== phoneNumber);
      const updatedTrustedNumbers = whatsappConfig.trustedNumbers.filter(
        (p) => p !== phoneNumber
      );
      const updatedBlacklistedNumbers =
        whatsappConfig.blacklistedNumbers.filter((p) => p !== phoneNumber);
   ...
   */
  async recordDetectionOverride(
    override: Omit<DetectionOverride, 'id' | 'createdAt'>
  ): Promise<void> {
    try {
      ...
      const contactType = this.modeToContactType(
        override.overriddenTo as DetectionMode
      );
      await this.updateContactClassification(
        override.whatsappId,
  ...
  async getDetectionAccuracy(instanceId: string): Promise<number> {
    try {
      ...
      const typedAnalytics = analytics as AnalyticsRecord[];

      typedAnalytics.forEach((day) => {
  ...
  private async updateContactClassification(
    ...
    try {
      const whatsappRecord = await prisma.professionalWhatsApp.findFirst({
        where: {
          OR: [{ instanceId: whatsappId }, { id: whatsappId }],
        },
      });
  ...
  private async getLatestOverride(
    ...
    try {
      ...
      const override =
        await prisma.professionalWhatsAppDetectionOverride.findFirst({
          where: {
            whatsappId: whatsappRecord.id,
            phoneNumber,
          },
