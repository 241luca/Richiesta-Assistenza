...
async function diagnoseWhatsApp() {
  ...
  if (fs.existsSync(servicePath)) {
    ...
    // Verifica anche la cartella token
    const tokenPath = './backend/tokens/assistenza-wpp';
    if (fs.existsSync(tokenPath)) {
      console.log('   Token salvati trovati ✓');
      console.log('   (WhatsApp era già stato connesso prima)');
    }
  } else {
