...
router.get('/status', authenticate, requireRole(['ADMIN', 'SUPER_ADMIN']), async (req: any, res: any) => {
  try {
    ...
    // IP bloccati (assumiamo che siano in una tabella o in metadata)
    // NOTA: La tabella blockedIp non esiste, quindi mettiamo 0
    const blockedIps = 0; // Tabella non esistente nel DB attuale
    ...
    res.json(ResponseFormatter.success({
      ...
      blockedIps,
      rateLimitHits,
      lastIncident: lastIncident?.timestamp.toISOString(),  // Cambiato da createdAt a timestamp
      events
    }));
