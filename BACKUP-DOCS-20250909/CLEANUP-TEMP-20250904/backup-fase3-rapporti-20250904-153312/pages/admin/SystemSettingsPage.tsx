...
export default function SystemSettingsPage() {
  ...
  const { data: settingsData, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/admin/system-settings'),
    staleTime: 30 * 1000
  });
...
function SettingRow({ setting, onEdit }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (newValue: string) => 
      api.put(`/admin/system-settings/${setting.key}`, { value: newValue }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/admin/system-settings/${setting.key}`),
    onSuccess: () => {
...
function FooterPreview() {
  const { data: footerConfig } = useQuery({
    ...
    queryFn: () => api.get('/admin/system-settings/footer/config'),
    staleTime: 60 * 1000
  });
...
function CreateSettingModal({ onClose, onSuccess }) {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => api.post('/admin/system-settings', data),
    onSuccess: () => {
...
function EditSettingModal({ setting, onClose, onSuccess }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => api.put(`/admin/system-settings/${setting.key}`, data),
    onSuccess: () => {
