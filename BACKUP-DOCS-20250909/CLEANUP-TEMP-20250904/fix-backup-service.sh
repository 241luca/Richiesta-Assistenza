...
echo "🔧 Fixing Backup Service..."

# Backup del file originale
cp backend/src/services/backup.service.ts backend/src/services/backup.service.ts.backup-$(date +%Y%m%d-%H%M%S)

# Crea il file corretto
cat > backend/src/services/backup.service.ts << 'EOF'
// Servizio di backup FUNZIONANTE
...
class BackupService {
  ...
  async listBackups(filters: any = {}): Promise<any> {
    try {
      ...
      // Costruisci where clause corretta
      const where: any = {
        ...whereFilters,
        deletedAt: null,
      };
  ...
  private async getBackupStats() {
    const [totalCount, completedCount, failedCount, totalSizeResult] = await Promise.all([
      ...
      prisma.systemBackup.count({ where: { status: BackupStatus.COMPLETED, deletedAt: null } }),
      prisma.systemBackup.count({ where: { status: BackupStatus.FAILED, deletedAt: null } }),
      prisma.systemBackup.aggregate({
        where: { status: BackupStatus.COMPLETED, deletedAt: null },
        _sum: { fileSize: true },
      }),
    ]);
...
# Installa archiver se non presente
cd backend
npm install --save archiver @types/archiver

echo "✅ Backup Service fixed!"
echo ""
echo "🔄 Restarting backend..."
# Il backend dovrebbe riavviarsi automaticamente con nodemon

echo ""
echo "📝 Summary of fixes:"
echo "1. ✅ Fixed listBackups query (moved limit/offset out of where)"
echo "2. ✅ Fixed database backup (check if tables exist)"
echo "3. ✅ Fixed compression using archiver instead of fs.createWriteStream"
echo "4. ✅ Fixed checksum calculation (check if file not directory)"
echo "5. ✅ Added BigInt serialization for JSON"
echo ""
echo "🎉 Backup service should now work correctly!"
