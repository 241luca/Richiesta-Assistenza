...
async function testDirect() {
  ...
  try {
    ...
    if (error.response?.status === 401) {
      ...
      console.log('\n🔑 IMPORTANTE:');
      console.log('   Evolution ha DUE tipi di chiavi:');
      console.log('   1. GLOBAL API KEY (nel docker-compose.yml)');
      console.log('   2. Instance Token (per ogni istanza)');
      console.log('   ');
      console.log('   Per le API devi usare la GLOBAL API KEY!');
    } else if (error.response?.status === 404) {
      console.log('\n⚠️  L\'istanza "assistenza" non esiste!');
