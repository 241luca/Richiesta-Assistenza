...
# ==============================================================================
# Analizza schema.prisma per trovare relazioni con e senza @relation
# ==============================================================================
...
for model in $MODELS; do
    ...
    while IFS= read -r line; do
        ...
        # Check if line contains a relation to another model
        # Pattern: starts with spaces, then field name, then model name (starting with uppercase)
        # Modified to handle arrays properly
        if echo "$line" | grep -qE "^[[:space:]]+[a-zA-Z_][a-zA-Z0-9_]*[[:space:]]+[A-Z][a-zA-Z0-9_]*(\[\]|\?)?[[:space:]]"; then
            ...
            FIELD_TYPE=$(echo "$line" | sed 's/^[[:space:]]*//' | awk '{print $2}')
    ...
    if [ $MODEL_WITH -gt 0 ] || [ $MODEL_WITHOUT -gt 0 ]; then
        ...
        if [ $MODEL_WITH -gt 0 ]; then
            ...
            while IFS=: read -r field type relation; do
                if [ ! -z "$relation" ]; then
                    echo -e "      ${GREEN}✅${NC} ${field}: ${type} ${CYAN}@relation(\"${relation}\")${NC}"
                else
...
if [ $TOTAL_WITHOUT_RELATION -gt 0 ] && [ ! -z "$MODELS_WITH_ISSUES" ]; then
    ...
    # Create a summary of issues per model
    for model in $MODELS_WITH_ISSUES; do
        # Re-process to count issues for this specific model
        MODEL_CONTENT=$(awk "/^model $model {/,/^}/" "$SCHEMA_FILE")
        COUNT=0
        ...
        while IFS= read -r line; do
            ...
            if [[ "$line" =~ ^[[:space:]]*@@ ]] || [[ "$line" =~ ^model ]] || [[ "$line" == "}" ]]; then
                continue
            fi
